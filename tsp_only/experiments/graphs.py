...
def plot_trajectory_length_comparison(vlm_lengths, mp_lengths, deviations, images, file_path="results/traj_bar_chart.png"):
    ...
    plt.bar(x - 0.2, vlm_lengths, width=0.4, label='VLM', align='center', alpha=0.8, color='#3C78D8')  
    plt.bar(x + 0.2, mp_lengths, width=0.4, label='MP', align='center', alpha=0.8, color='#D47F27') 
    ...
    plt.title("Comparison of Trajectory Lengths from VLM (TSP) and MP with Deviations", fontsize=18)  
...
def plot_rmse_boxplot(knn_errors, dtw_rmse, interp_rmse, file_path="results/error_box_plot.png"):
    ...
    for median_line in box['medians']:
        median_line.set_color('#D47F27')
        median_line.set_linewidth(2)

    plt.yscale('log')  # Apply logarithmic scale to y-axis
    plt.title("Error Distribution b/w VLM (TSP) and MP Methods (Log Scale)", fontsize=16)  
